...
**문서 성격** — 이 문서는 **설계 근거**다. 당일 진행 스크립트(타임라인 · 대사 · 실패 대응)는 [GUIDE_0_overview.md](GUIDE_0_overview.md) 및 `GUIDE_1.md`~`GUIDE_7.md` 를 본다.

**경로 기준** — 저장소 구조가 평탄화되어 `module_N/exercise/exercise_N-M/` · `module_N/LAB/` 계층은 존재하지 않는다. 본 문서의 경로는 실경로 기준으로 갱신되었다.
...
| 4 | LAB_4-3 | skill-creator 로 개선 | LAB_4-1 결과물 리팩터 | 스킬이 **스킬을 만든다** |
...
| 5 | LAB_5-3 | 실패 사례 | 충돌하는 지시로 결과 뒤엉킴 재현 | 오케스트레이션 **주의사항** |
| 6 | LAB_6-1 | 작업일지 훅 | Stop → `worklog.md` 자동 append | Hook 은 **모델 판단 없이** 실행 |
| 6 | LAB_6-2 | 회귀테스트 훅 | Stop → lint/test → 실패 시 stdin 재투입 | **자가 수정 루프** |
| 6 | LAB_6-3 | 가드레일 훅 | `rm -rf` · `.env` 접근 차단 (PreToolUse) | 하네스의 **안전장치** |
| 7 | — | 캡스톤 | LAB 없음, 전 구간 자율 | 하네스 **총동원** |
...
- 산출물 : [QUEST_1.md](../module_1/QUEST_1.md)
...
| ID | 실습 | 핵심 장면 |
| --- | --- | --- |
| LAB_2-1 | 타임머신 | 잘못된 지시로 코드 붕괴 → `/rewind` 복구 |
...
- 산출물 : [QUEST_2.md](../module_2/QUEST_2.md) · [PRD](../module_2/PRD_fleet_console.md)
...
| 제공물 | 경로 | 내용 |
| --- | --- | --- |
| 기준본 | `module_3/exercise_3_baseline/` | **React + Vite + TS**, FR-01~07 구현 (65점 수준). `node_modules` 포함 배포 → 즉시 `npm run dev` |
| 변경요구서 | `module_3/CR_v1.1.md` | CR-01 · CR-02 · CR-03 |
| 강사 가이드 | `instructor/GUIDE_3.md` | LAB_3-1~3-3 진행 스크립트 |
| 템플릿 | `module_3/CLAUDE_template.md` | 빈칸 CLAUDE.md |
| v1.1 데이터 | `exercise_3_baseline/public/data/vehicles_v11.json` | `CHARGING` 4대 포함 |
...
| ID | 실습 | 핵심 장면 |
| --- | --- | --- |
| LAB_3-1 | 하네스 없는 세상 | **빈 CLAUDE.md** 상태로 CR-01 투입 → 위반 실황 중계 |
| LAB_3-2 | 하네스 5요소 지도 | CLAUDE.md · Skill · Tool · SubAgent · Hook 개입 시점 비교 |
| LAB_3-3 | 규칙 추출 | LAB_3-1 에서 나온 위반 → 검증 가능한 규칙 문장으로 변환 |

### exercise_3-1 「나만의 CLAUDE.md 채우기 + 동일 CR 재실행」 (10분)

- 산출물 : [QUEST_3-1.md](../module_3/QUEST_3-1.md) · [CLAUDE_template.md](../module_3/CLAUDE_template.md)

| 단계 | 할 일 |
| --- | --- |
| 1 | `CLAUDE_template.md` 빈칸 채우기 (개요 · 스택 · 컨벤션 · 유의사항 · Do/Don't) |
| 2 | `/rewind` 로 CR-01 이전 상태 복원 |
| 3 | **완전히 동일한 프롬프트**로 CR-01 재실행 |
| 4 | Before / After 나란히 비교 |
...
- 산출물 : [QUEST_3-2.md](../module_3/QUEST_3-2.md) · [CR_v1.1.md](../module_3/CR_v1.1.md)
...
| LAB_4-3 | `skill-creator` 리팩터 | 10분 | LAB_4-1 결과물 투입 → 구조·트리거 개선 | 스킬이 **스킬을 만든다** |
...
| 경로 | 용도 | 상태 |
| --- | --- | --- |
| [GUIDE_4.md](GUIDE_4.md) | 강사 진행 가이드 | ✅ |
| `dtc-explainer/` (완성본) | LAB_4-1 시연용 | ❌ **저장소 미포함** — 강사 자산. 코드는 `instructor/GUIDE_4.md` 참조 |
| `dtc-explainer-bad/` (부실본) | LAB_4-2 함정 시연용 | ❌ **저장소 미포함** — 강사 자산. 코드는 `instructor/GUIDE_4.md` 참조 |
| [QUEST_4.md](../module_4/QUEST_4.md) | 퀘스트 문서 | ✅ |
| [CHARGER_BRIEF.md](../module_4/CHARGER_BRIEF.md) | EV 충전기 예약 요구 브리프 | ✅ |
| `module_4/skills/prd-creator/` | ⬜ **스텁** — `description` · 지침 빈칸 | ✅ |
| `module_4/skills/grill-me/` | 심문 스킬 (한국어 번역본, frontmatter `name: grilling`) | ✅ **저장소 포함** |
| `module_4/skills/skill-creator/` | Anthropic 배포본 | ✅ **저장소 포함** |
...
| 단계 | 할 일 | |
| --- | --- | --- |
| 1 | `prd-creator` **스텁 채우기** — `description` + 지침 (PRD 목차·필수 섹션·금지어) | 3분 |
| 2 | 내 스킬 호출 → `PRD_charger_v1.md` 생성 | 2분 |
| 3 | `grill-me` 로 심문 → 허점 리스트 확보 | 2분 |
| 4 | 지적 **3건 이상** 반영 → `PRD_charger_v2.md` | 3분 |
...
- **시간 방어 장치** : 스킬 폴더·frontmatter 골격은 제공. 교육생이 쓰는 것은 `description` 과 지침 본문뿐
...
| ID | 실습 | | 소재 | 전달 원리 |
| --- | --- | --- | --- | --- |
| LAB_5-1 | 레포 탐색 대결 | 10분 | `exercise_3_baseline` 를 ⓐ단일 세션 ⓑ병렬 3 으로 파악 | 서브에이전트가 **내 컨텍스트를 대신 태움** |
| LAB_5-2 | 역할 부여 | 10분 | `.claude/agents/` 에 보안·성능·컨벤션 3종 정의 | **시야 제한**이 리뷰 품질을 올림 |
| LAB_5-3 | 실패 사례 | (5-2 내 3분) | 3 에이전트에 **같은 파일 수정** 지시 → 결과 뒤엉킴 | 병렬은 **읽기에 강하고 쓰기에 약함** |
...
| 경로 | 용도 | 상태 |
| --- | --- | --- |
| [GUIDE_5.md](GUIDE_5.md) | 강사 진행 가이드 | ✅ |
| [ANSWER_KEY.md](../module_5/ANSWER_KEY.md) | 정답지 (강사 전용, 배포 금지) | ✅ |
| `module_5/agents/*.md` | 보안 · 성능 · 컨벤션 리뷰어 정의 3종 (LAB · 실습 공용) | ✅ |
| [QUEST_5.md](../module_5/QUEST_5.md) | 퀘스트 문서 | ✅ |
| — | (사본 불필요 — 위 3종을 공용) | — |
| `module_5/review-target/telemetry_api.py` | 결함 1~5 + 보너스 2종 | ✅ |
| `module_5/review-target/web/` | 결함 6~8 (fleet-console 스냅샷) | ✅ |
...
| 리뷰 대상 | ⓐ `telemetry_api.py` (신규, 백엔드) + ⓑ `exercise_3_baseline/src/` (3교시 레포) |
| 심어둔 결함 | **8종** (아래 배치표) |
...
| 6 | 불필요 전체 재계산 | `exercise_3_baseline/src/` | 성능 |
| 7 | 네이밍·매직넘버 위반 | `exercise_3_baseline/src/` | 컨벤션 |
| 8 | 데드코드 | `exercise_3_baseline/src/` | 컨벤션 |

- 흥미 포인트 : 실시간 스코어보드 **"8개 중 몇 개?"**
- 오탐 감점 존재 → "많이 찍기" 전략 차단

---

## 6교시 : Hooks (16:00~16:50)

> **훅 스크립트 언어 : Node.js 확정** (`.claude/hooks/*.js`)
...
| LAB_6-3 | 가드레일 훅 | (6-2 내 5분) | `PreToolUse` | 하네스의 **안전장치** (차단 = `exit 2`) |

- LAB_6-2 결정타 : 일부러 타입 오류를 심음 → 훅이 잡음 → Claude 가 **스스로** 고침
- 대상 레포 : `exercise_3_baseline` (`npx tsc -b` 를 lint/test 로 사용)

### 제공물

| 경로 | 용도 | 상태 |
| --- | --- | --- |
| [GUIDE_6.md](GUIDE_6.md) | 강사 진행 가이드 | ✅ |
| `worklog.js` | Stop — 작업일지 append | ❌ **저장소 미포함** — 코드는 `instructor/GUIDE_6.md` (미검증) |
| `regression.js` | Stop — `npx tsc -b` → 실패 시 `exit 2` | ❌ **저장소 미포함** — 코드는 `instructor/GUIDE_6.md` (미검증) |
| `guard.js` | PreToolUse — 위험 명령·경로 차단 | ❌ **저장소 미포함** — 코드는 `instructor/GUIDE_6.md` (미검증) |
| `module_6/settings.example.json` | 훅 등록 예시 | 🔶 위 3종을 참조 중 — **파일이 없으면 그대로는 동작하지 않음** |
| [QUEST_6.md](../module_6/QUEST_6.md) | 퀘스트 문서 | ✅ |
| `module_6/hooks/{A,B,C}_*.js` | ⬜ 트랙별 스텁 3종 | ✅ |

### exercise_6-1 「신입사원 퀘스트 6 : 나만의 훅」 (10분, 택1)

| 트랙 | 훅 | 시점 | 난이도 | 판정 방법 |
| --- | --- | --- | --- | --- |
| A | 위험 명령 차단 (`rm -rf` · `.env` 읽기) | `PreToolUse` | ⭐⭐ | 금지 명령 요청 → 차단 로그 |
...
- [GUIDE_7.md](GUIDE_7.md) — LAB 없음. 트랙 배정 · 순회 코칭 · 채점 · 총정리
- **30분 시점 강제 전환** : "손 멈추세요" → 리뷰·훅 확인 단계

### exercise_7-1 「캡스톤 : 하네스 총동원」 (40분 + 발표 3분)

- [QUEST_7.md](../module_7/QUEST_7.md)
- 스택 **자유** · `exercise_3_baseline` 복사 출발 허용 · 오전·오후 산출물 **반입 허용**

** 트랙 7종**

| 난이도 | 트랙 | 프로젝트 | 데이터 | 핵심 |
| --- | --- | --- | --- | --- |
| 🟢 하 | H1 | [DTC 진단코드 사전](../module_7/tracks/H1_dtc_dictionary/BRIEF.md) | 코드 60건 | 제조사 정의 코드 `null` 처리 |
| 🟢 하 | H2 | [신입 온보딩 용어집](../module_7/tracks/H2_glossary/BRIEF.md) | 용어 36건 | 통합검색 · 연관 이동 |
| 🟢 하 | H3 | [EV 충전소 현황판](../module_7/tracks/H3_charger_board/BRIEF.md) | 충전기 8 · 세션 20 | 진행 세션 `null` 처리 |
| 🟢 하 | H4 | [차량 부품 재고](../module_7/tracks/H4_parts_inventory/BRIEF.md) | 부품 80 · 차종 12 | 재고 부족 경계값 |
| 🟡 중 | M1 | [EV 충전기 예약](../module_7/tracks/M1_charger_reservation/BRIEF.md) | 충전기 8 · 예약 24 | **중복 예약 차단** |
| 🟡 중 | M2 | [차량 정비 이력](../module_7/tracks/M2_maintenance_history/BRIEF.md) | 이력 90 · 차량 30 | **지연 판정(거리+기간)** |
| 🟡 중 | M3 | [스마트팩토리 모니터링](../module_7/tracks/M3_factory_monitor/BRIEF.md) | 설비 12 · 실적 168 | **가동률 정의 통일** |
...
| | SubAgent | 15 |
| | Hook | 15 |
| 기능 25 | 필수 FR / 데이터 정확성 / 실행 가능성 | 15 / 5 / 5 |
...
| 5 | `module_3/exercise_3_baseline/` | 병렬 탐색 + 리뷰 대상 |
| 6 | `module_3/exercise_3_baseline/` | 회귀테스트 훅 대상 |
| 7 | 하네스 4종 전부 | 캡스톤 필수 조건 (오전·오후 산출물 반입 허용) |
| 7 | 2교시 `vehicles`·`trips` | M2 트랙 데이터로 동봉 |
| 7 | `exercise_3_baseline` | 캡스톤 출발점으로 복사 허용 |
...
| 항목 | 현재 상태 | 신뢰도 | 비고 |
| --- | --- | --- | --- |
| `grill-me` | ✅ 저장소 포함 (`module_4/skills/grill-me/`) | 🟢 높음 | frontmatter `name: grilling` — 폴더명과 불일치. 호출 확인 필요 |
| `skill-creator` | ✅ 저장소 포함 (`module_4/skills/skill-creator/`) | 🟢 높음 | Anthropic 배포본 |
| **`dtc-explainer` 2종 (4교시 LAB)** | ❌ **저장소 미포함** | 🔴 낮음 | 강사 자산. 코드는 `GUIDE_4.md` 수록. **사전 제작 필수** |
| **LAB 훅 3종 (6교시)** | ❌ **저장소 미포함** — `settings.example.json` 이 참조 중 | 🔴 낮음 | 코드는 `GUIDE_6.md` 수록. **실행 미검증. 사전 제작·검증 필수** |
| 교육생 훅 스텁 3종 | ✅ `module_6/hooks/{A,B,C}_*.js` + `_stdin.js` | 🟢 높음 | 스텁 상태 (⬜ 빈칸) |
| 훅의 **세션 내 실제 발동** | 미검증 | 🟡 중간 | `.claude/settings.json` 등록 후 강사 PC 에서 리허설 필요 |
| 서브에이전트 정의 파일 스키마 | `name`·`description`·`tools` frontmatter 기준으로 작성 | 🟡 중간 | 강사 PC 버전에서 호출 확인 필요 |
| `/rewind` 파일 복원 범위 | 미검증 | 🟡 중간 | 버전별 차이. 2·3교시 대사가 달라짐 |
| 대용량 텔레메트리 로그 (LAB_2-2) | ❌ 미제작 | 🟡 중간 | 현재 `trips.json`(1,135줄) 로 대체 — 체감 약할 수 있음 |
| 캡스톤 기대값 (H4 · M2 · M3) | 설계안 기재값, 데이터 대조 미실시 | 🟡 중간 | 채점 전 검산 필요. H1 · H3 은 확인 완료 |
...
| 8 | 5교시 리뷰 대상 | `telemetry_api.py` (신규) **+** `exercise_3_baseline` 레포 동시 제공 |
| 9 | 6교시 훅 언어 | **Node.js** (`.claude/hooks/*.js`) |
| 10 | 캡스톤 트랙 | 난이도 하 4종(H1~H4) + 중 3종(M1~M3) = **7트랙** |
| 11 | 캡스톤 스택 | **자유** + `exercise_3_baseline` 복사 허용 |
