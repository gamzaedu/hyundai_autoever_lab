...
1967년 설립된 현대자동차는 현재 완성차 제조사이자 모빌리티 기업으로 스스로를 규정하며, 공식 연혁은 울산 조립공장에서 현재의 IONIQ 전기차에 이르는 전환 과정을 기록한다. 캡처된 한국어 제품 화면에서 이 광범위한 자동차 정체성은 절제된 인터페이스로 구현된다 — 흑백 텍스트, 딥 네이비 액션 컬러(`#002c5f`), 로드된 HyundaiSans 폰트 패밀리. 문서화된 차량 디자인 방향성인 Sensuous Sportiness는 2018년 이후 현대의 디자인 철학이며, 감성적 매력을 구조·비례·스타일링·기술과 연결한다. 본 레퍼런스는 이 공식 차량 디자인 맥락과 웹 주장(claim)을 분리한다 — 즉, 캡처된 KR 제품 UI를 기술할 뿐, 범용 현대 디자인 시스템을 기술하지 않는다.
...
- 로드된 HyundaiSans KR 패밀리는 현대가 라이선스를 제공한 에셋에 한해 사용.
- 본 레퍼런스 재사용 시 컴포넌트의 표면(surface)과 상태 경계 유지.

### Don't

- 틸·시안·원형 챗봇을 범용 액센트 또는 플로팅 액션 시스템으로 취급 ❌
- 시스템 폰트를 대체 사용하면서 Hyundai Sans로 표기 ❌
...
3. **모빌리티 UX의 가독성.** 현대 공식 Hyundai Sans UI 설명은 주행 환경에서의 가독성과 위계를 강조. *UI 함의:* 이는 서체의 브랜드 맥락을 뒷받침할 뿐, 대체 사용이나 웹 라이선스를 뒷받침하지 않음.
...
- **라이선스 · 배포 경계:** 제공된 캡처에 폰트 소스 URL 기록 없음. 본 검토에서 KR 폰트 파일에 대한 1차 출처 웹폰트 라이선스 확인 불가. 로드된 패밀리는 명칭과 관측 메트릭으로 기술 가능하나, 다운로드 가능한 에셋이나 재사용 라이선스는 주장하지 않음.

### 관측된 위계

| 역할 | 패밀리 | 크기 | 두께 | 행간 | 자간 | 화면 |
|------|--------|------|------|------|------|------|
| H2 | HyundaiSansHeadKR | 44px | 400 | 58px | -0.4px | home |
| 본문/리스트 | HyundaiSansTextKR | 16px | 400 | 18.4px | normal | 3개 제품 화면 전체 |
| 차량 액션 | HyundaiSansTextKR | 16px | 500 | 18.4px | -0.4px | 카탈로그 · IONIQ 6 |
| 주 내비 트리거 | HyundaiSansHeadKRR | 16px | 400 | 30px | -0.4px | 3개 제품 화면 전체 |
| 인라인 외부 링크 | HyundaiSansHeadKR | 14px | 500 | 14px | -0.4px | 3개 제품 화면 전체 |
...
- 텍스트 : `#000000`
- Radius : 0px
- 폰트 : 16px / 400 / HyundaiSansHeadKRR
...
- Radius : 0px
- Padding : 0px 13px
- 사용 : `home::[captured element]` / 3개 제품 화면 전체에서 관측, 3회. 시스템 Arial 사용, HyundaiSans UI 토큰이 아닌 푸터 유틸리티 크롬으로 유지.
...
출처에 근거한 값만 적용 — 네이비 차량 액션(`#002c5f`, 흰색 텍스트, 0px radius, 16px/500 HyundaiSansTextKR)은 2개 차량 제품 화면에서만 뒷받침됨. 본 레퍼런스로부터 카드·인풋·에러 상태·모션 사양을 도출하지 말 것.
