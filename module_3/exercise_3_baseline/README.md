...
| FR-09 | 알람 토스트/이력 | ❌ 미구현 |
| FR-10 | 반응형 | 부분 |
| — | 디자인·테마 (색·타이포·상태 표현) | ❌ 미적용 |

> `src/index.css` 는 **레이아웃만 남긴 기준 스타일**입니다. 색상·타이포그래피·상태 표현(상태 배지, 알람, SOC 경고 등)은
> 비어 있으며, `DESIGN.md` 를 근거로 직접 작성합니다. JSX 의 `className` 은 모두 유지되어 있으므로 CSS 만 채우면 됩니다.
> 폰트는 **CDN 금지** — `npm install pretendard` 로 설치합니다(`DESIGN.md` §3 「폰트 설치 · 로드」 참고).
